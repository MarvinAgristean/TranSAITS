...
def brits_unwrap(config):
    ...
    -------
        BRITS model:
            a new BRITS model using the parameters meantioned above
    """
    return BRITS(config['seq_len'], config['feat_num'], config['rnn_hidden_size'], MIT=True, device=config['device'])\
        .to(config['device'])

def brits_training_loop(loader, model, optimizer, data_set, missing_mask, indicating_mask, deltas, device):
    """
    Parameters
    ----------
    loader: DataLoader
    ...
    model: BRITS
        BRITS model that shall be trained here
    optimizer:
    ...
    for indice in loader:
        counter = counter + 1
        data_batch = data_set[indice].type(torch.float32)
        missing_mask_batch = missing_mask[indice].type(torch.float32)
        indicating_mask_batch = indicating_mask[indice].type(torch.float32)
        deltas_batch = deltas[indice].type(torch.float32)
        ...
        input = {
            'X_holdout': data_batch * indicating_mask_batch,
            'indicating_mask': indicating_mask_batch,
            'forward': {
                'X': data_batch * missing_mask_batch,
                'missing_mask': missing_mask_batch,
                'deltas': deltas_batch
                },
            'backward': {
                'X': data_batch_backwards*missing_mask_batch_backwards,
                'missing_mask': missing_mask_batch_backwards,
                'deltas': deltas_batch_backwards
                }
            }
        output = model.forward(input, 'train')
        loss = output['consistency_loss'] + output['reconstruction_loss'] + output['imputation_loss']
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        mean_loss += loss
...
def brits_evaluate(model, data_set, missing_mask, indicating_mask, deltas):
    """
    Parameters
    ----------
    model: BRITS
    ...
    with torch.no_grad():
        input = {
            'forward': {
                'X': data_set * missing_mask,
                'missing_mask': missing_mask,
                'deltas': deltas
            },
            'backward': {
                'X': data_set_backwards*missing_mask_backwards,
...
def brits_train(config, enable_tracking = False, early_stopping = True):
    """
    Parameters
    ----------
    config: dictionary containing:
    ...
    for epoch in range(config['epochs']):
        model.train()
        brits_training_loop(train_loader, model, optimizer, config['train_set'], config['missing_mask_train'],
                            config['indicating_mask_train'], config['deltas_train'], config['device'])
        model.eval()
        with torch.no_grad():
            val_evaluation = brits_evaluate(model, config['validation_set'], config['missing_mask_validation'],
...
def brits_transfer_learning(config, enable_tracking = False, early_stopping = True):
    """
    Parameters
    ----------
    config: dictionary containing:
        'path_pretrained':
            the datapath to a pretrained BRITS model, that shall be finetuned in this function
        'device':
              which device should be used
        'optimizer_name':
              name of the optimizer, that shall be usedin training
    ...
    -------
        model:
            an BRITS model, finetuned by the parameters above
        tracker: dictionary
            contains the metrics MAE, RMSE and MRE of the validation set and the models estimation for every epoch of training
    """
    ...
    for epoch in range(config['epochs']):
        model.train()
        brits_training_loop(train_loader, model, optimizer, train_set, missing_mask_train,
                            indicating_mask_train, deltas_train, device)
        model.eval()
        with torch.no_grad():
            val_evaluation = brits_evaluate(model, validation_set, missing_mask_validation, indicating_mask_validation, deltas_validation)
        early_stopper.report(val_evaluation['imputation mae'].item())
        if enable_tracking:
            tracker[epoch] = val_evaluation
        if early_stopper.should_stop() & early_stopping:
